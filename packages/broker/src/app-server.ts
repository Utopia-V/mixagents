...
export function renderRuntimeConfig(route: RouteConfig): string {
  const lines = [
  ...
  lines.push(
    ...
    `name = ${tomlString(route.providerName)}`,
  ...
  if (route.envKey) {
    ...
    lines.push(
      `env_key_instructions = ${tomlString(`Set ${route.envKey} outside Codex before starting Broker.`)}`,
    );
  }
...
export function runtimeIdFor(route: RouteConfig, access: Access): string {
...
export function encodeAgentId(runtimeId: string, threadId: string): string {
...
export function parseAgentId(agentId: string): { runtimeId: string; threadId: string } {
...
function filteredChildEnvironment(
  route: RouteConfig,
  runtimeDirectory: string,
  environment: NodeJS.ProcessEnv,
): NodeJS.ProcessEnv {
  const result: NodeJS.ProcessEnv = { CODEX_HOME: runtimeDirectory };
  const safeNames = [
    "PATH",
    "HOME",
    "USER",
...
async function atomicWrite(filePath: string, contents: string, mode = 0o600): Promise<void> {
  const temporaryPath = `${filePath}.${process.pid}.tmp`;
  await writeFile(temporaryPath, contents, { encoding: "utf8", mode });
  await rename(temporaryPath, filePath);
}

function extractTurnOutput(turn: ThreadTurn | undefined): string | undefined {
...
function extractTurnError(turn: ThreadTurn | undefined): string | undefined {
  if (!turn?.error) {
    return undefined;
  }
...
function statusFromTurn(turn: ThreadTurn | undefined): AgentSnapshot["status"] {
  switch (turn?.status) {
    case "inProgress":
      return "running";
    case "interrupted":
...
function threadFromResult(result: unknown): ThreadRecord {
  ...
  return result.thread as unknown as ThreadRecord;
}

function turnFromResult(result: unknown): ThreadTurn {
  ...
  return result.turn as unknown as ThreadTurn;
}

export class AppServerRuntime extends EventEmitter {
  readonly spec: RuntimeSpec;
  readonly #environment: NodeJS.ProcessEnv;
  readonly #agents = new Map<string, InternalAgent>();
  #child: ChildProcessWithoutNullStreams | undefined;
  #peer: JsonLinePeer | undefined;
  #starting: Promise<void> | undefined;
  #stopping = false;
  #revision = 0;

  constructor(spec: RuntimeSpec, environment = process.env) {
    super();
    this.spec = spec;
  ...
  async #startProcess(): Promise<void> {
    ...
    await atomicWrite(
      path.join(this.spec.directory, RUNTIME_CONFIG_FILE),
      renderRuntimeConfig(this.route),
    );
    await atomicWrite(
      path.join(this.spec.directory, RUNTIME_METADATA_FILE),
      `${JSON.stringify(this.spec.metadata, null, 2)}\n`,
    );
    await chmod(path.join(this.spec.directory, RUNTIME_CONFIG_FILE), 0o600).catch(
      () => undefined,
    );
    await chmod(path.join(this.spec.directory, RUNTIME_METADATA_FILE), 0o600).catch(
      () => undefined,
    );
    ...
    child.once("exit", (code, signal) => {
      const reason = new Error(
        `App Server ${this.runtimeId} exited (${code ?? signal ?? "unknown"})`,
      );
    ...
    try {
      ...
      throw new BrokerError(
        "runtime_start_failed",
        `Cannot start App Server for route ${this.route.id}: ${errorMessage(error)}`,
      );
    }
  }
  ...
  async startAgent(task: string, cwd: string): Promise<AgentSnapshot> {
    await this.start();
    const response = await this.#request("thread/start", {
      ...
      developerInstructions:
        "You are a Broker-managed worker. Complete the assigned task in the supplied workspace and return a concise final result to the controller. Do not spawn subagents or invoke Broker.",
      ephemeral: false,
    });
    const thread = threadFromResult(response);
    const effectiveProvider =
      isRecord(response) && typeof response.modelProvider === "string"
        ? response.modelProvider
        : thread.modelProvider;
    const effectiveModel =
      isRecord(response) && typeof response.model === "string" ? response.model : undefined;
    if (effectiveProvider !== this.route.provider || effectiveModel !== this.route.model) {
      throw new BrokerError(
        "route_mismatch",
        `App Server created ${effectiveProvider ?? "unknown"}/${effectiveModel ?? "unknown"}, expected ${this.route.provider}/${this.route.model}`,
      );
    }
  ...
  async recover(threadId: string): Promise<InternalAgent> {
  ...
  async send(threadId: string, message: string): Promise<AgentSnapshot> {
    ...
    if (agent.interactions.length > 0) {
      throw new BrokerError(
        "interaction_pending",
        "The worker is waiting for host interaction; call wait_agent first",
      );
    }
    delete agent.output;
    delete agent.error;
  ...
  async interrupt(threadId: string): Promise<{ previousStatus: AgentSnapshot["status"]; agent: AgentSnapshot }> {
    const agent = await this.recover(threadId);
    ...
    do {
      ...
      for (const item of response.data) {
        if (isRecord(item) && typeof item.id === "string") {
          threads.push(item as unknown as ThreadRecord);
        }
      }
  ...
  #queueServerRequest(method: string, params: unknown): Promise<unknown> {
    if (!isRecord(params) || typeof params.threadId !== "string") {
      throw new BrokerError(
        "unsupported_interaction",
        `App Server request ${method} has no worker thread identity`,
      );
    }
    ...
    if (!agent) {
      throw new BrokerError(
        "unknown_interaction",
        `App Server requested ${method} for an unknown Broker thread`,
      );
    }
  ...
  #handleNotification(method: string, params: unknown): void {
    ...
    if (method === "turn/completed" && isRecord(params.turn) && typeof params.turn.id === "string") {
      const turn = params.turn as unknown as ThreadTurn;
...
export class RuntimeManager {
  ...
  async forRoute(route: RouteConfig, access: Access): Promise<AppServerRuntime> {
  ...
  async forAgent(agentId: string): Promise<{ runtime: AppServerRuntime; threadId: string }> {
  ...
  async all(): Promise<AppServerRuntime[]> {
  ...
  async #loadRuntime(runtimeId: string): Promise<AppServerRuntime> {
    const directory = path.join(this.#dataDir, "runtimes", runtimeId);
    const metadataPath = path.join(directory, RUNTIME_METADATA_FILE);
    let metadata: RuntimeMetadata;
    try {
      metadata = JSON.parse(await readFile(metadataPath, "utf8")) as RuntimeMetadata;
    } catch (error) {
      throw new BrokerError(
        "runtime_not_found",
        `Cannot read runtime ${runtimeId}: ${errorMessage(error)}`,
      );
    }
    if (
      metadata.version !== 1 ||
