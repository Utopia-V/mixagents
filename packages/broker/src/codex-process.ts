...
function managedWindowsExecutable(
  managedRoot: string,
  arch: string,
): string | undefined {
  ...
  try {
    ...
    const platformPackage = requireFromCodex.resolve(
...
function windowsScriptSpec(command: string): ProcessSpec {
  return {
    command,
    prefixArgs: [],
...
export function resolveCodexProcess(
  codexBin: string,
  ...
  const managedRoot = environmentValue(
    environment,
  ...
  if (commandShim) {
    const inferredManagedRoot = path.win32.join(
      path.win32.dirname(commandShim),
      "node_modules",
      "@openai",
      "codex",
    );
...
function windowsCommandToken(value: string): string {
  if (/[\0\r\n"%!]/u.test(value)) {
    throw new BrokerError(
      ...
      "The Windows Codex command contains characters that cannot be passed safely; configure codexBin with the full path to codex.exe",
    );
  }
  if (/[\s&|<>^()]/u.test(value)) {
    return `"${value}"`;
