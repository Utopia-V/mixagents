...
const TOOLS = [
    {
        ...
        description: "List configured provider/model routes, credential availability, and the selected backend. Makes no provider request.",
    ...
    },
    {
        ...
        description: "Create a Broker-managed App Server agent on one configured route and start its first turn. Sends the task to that provider and may incur cost.",
        inputSchema: {
            type: "object",
            properties: {
                ...
                cwd: {
                    type: "string",
                    description: "Absolute working directory. Broker uses a preauthorized root or asks the host to approve it for this connection.",
                },
                access: {
                    type: "string",
    ...
    },
    {
        ...
        description: "Steer a running managed agent or start a follow-up turn on the same agent thread.",
        inputSchema: {
            type: "object",
            properties: {
                agentId: { type: "string" },
    ...
    },
    {
        ...
        description: "Wait for one of one to eight managed agents to finish, fail, interrupt, or request host interaction.",
        inputSchema: {
            type: "object",
            properties: {
                agentIds: {
    ...
    },
    {
        ...
        description: "Interrupt the active managed-agent turn without destroying its reusable thread. Remote provider work may continue.",
        inputSchema: {
            type: "object",
...
function workspaceApprovalAccepted(result) {
    return (isRecord(result) &&
        result.action === "accept" &&
        isRecord(result.content) &&
        result.content.decision === "approve");
}
function waitForWorkspaceApproval(request, signal) {
    if (signal.aborted) {
        return Promise.reject(new BrokerError("spawn_cancelled", "spawn_agent was cancelled"));
    }
...
export class BrokerMcpServer {
    ...
    #options;
    #activeCalls = new Map();
    #approvedWorkspaceRoots = new Set();
    #pendingWorkspaceApprovals = new Map();
    #broker;
    #clientCapabilities = {};
    ...
    async #handleRequest(method, params, id) {
        if (method === "initialize") {
            ...
            return {
                ...
                instructions: "Use routes before dispatch and pass an absolute cwd. Broker keeps provider/model fixed and returns native-sized agent results.",
            };
        }
        if (!this.#initialized) {
    ...
    async #handleToolCall(params, requestId) {
        ...
        try {
            ...
            switch (params.name) {
                ...
                case "wait_agent":
                    const waitOptions = { signal: controller.signal };
                    if (this.#supportsElicitation()) {
                        waitOptions.elicit = (request) => this.#elicit(request);
                    }
                    return toolSuccess(await broker.waitAgents(args.agentIds, args.timeoutMs, waitOptions));
    ...
    #supportsElicitation() {
        return ((this.#protocolVersion === "2025-11-25" ||
    ...
    async #spawnAgent(broker, args, signal) {
        ...
        try {
            ...
            if (!this.#supportsElicitation()) {
                throw new BrokerError(error.code, `${error.message}; this MCP client cannot request workspace approval, so configure workspaceRoots in broker.json`);
            }
            const cwd = await resolveWorkspace(args.cwd);
            const approved = await this.#approveWorkspace(cwd, signal);
            if (!approved) {
                throw new BrokerError("workspace_approval_declined", `Workspace access was not approved for ${cwd}`);
            }
            if (signal.aborted) {
    ...
    async #approveWorkspace(cwd, signal) {
            ...
            request = (async () => {
                try {
                    return await this.#elicit({
                        mode: "form",
                        message: "Allow MixAgents Broker to use this directory and its subdirectories as an agent workspace for this Codex connection?\n\n" +
                            `Workspace: ${cwd}\n\n` +
                    ...
                    throw new BrokerError("workspace_approval_unavailable", `Could not request workspace access for ${cwd}: ${errorMessage(error)}`);
                }
            })();
