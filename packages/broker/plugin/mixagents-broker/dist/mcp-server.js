    ...
    async #handleRequest(method, params, id) {
        if (method === "initialize") {
            ...
            return {
                ...
                instructions: "Use routes before dispatch. Broker keeps provider/model fixed and returns native-sized agent results.",
            };
        }
        if (!this.#initialized) {
    ...
    async #handleToolCall(params, requestId) {
        ...
        try {
            ...
            switch (params.name) {
                ...
                case "spawn_agent":
                    return toolSuccess(await broker.spawnAgent({
                        route: args.route,
                        task: args.task,
                        cwd: args.cwd,
                        access: args.access,
