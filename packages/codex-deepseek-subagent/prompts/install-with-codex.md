...
3. Install exactly one agent file as
   ...
   - On Windows or Linux, use
     packages/codex-deepseek-subagent/agents/v4-flash-worker.toml. On Windows, a fully
     ...
     packages/codex-deepseek-subagent/agents/windows-live-env/v4-flash-worker.toml
     variant, replace it with the
     ...
     identity boundary.
   - On macOS, use
     packages/codex-deepseek-subagent/agents/v4-flash-worker.toml by default.
     Use packages/codex-deepseek-subagent/agents/macos-keychain/v4-flash-worker.toml
     only when I explicitly request
   ...
   from memory.
4. Install `packages/codex-deepseek-subagent/skills/use-v4-flash-worker` as
...
5. Install the platform handoff script under the stable personal directory
   ...
   - On macOS or Linux, require an available Python 3 and install
     `packages/codex-deepseek-subagent/hooks/plaintext_handoff.py`. If Python 3
...
6. Install one `SubagentStart` command Hook whose matcher is exactly
   ...
   `packages/codex-deepseek-subagent/hooks/hooks.*.example.json` as the
   structural source.
...
7. Merge `packages/codex-deepseek-subagent/snippets/AGENTS.md` into the personal
   AGENTS.md once, preserving the
...
9. Parse the final Hook source as JSON or TOML and verify its exact matcher,
   ...
   `packages/codex-deepseek-subagent/tests/plaintext-handoff.windows.ps1` on
   Windows, or
   `packages/codex-deepseek-subagent/tests/test_plaintext_handoff.py` on
   macOS/Linux.
...
11. Read back the installed configuration with any credential-like text
    ...
    `packages/codex-deepseek-subagent/prompts/quick-smoke-test.md` as the default
    checkout-free test; reserve
    `packages/codex-deepseek-subagent/prompts/smoke-test.md` for contributors
